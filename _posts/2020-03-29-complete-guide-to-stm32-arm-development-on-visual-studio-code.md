...
It's important to change the ```executable``` tag to point to your project's ```.elf``` file. The ```servertype``` tag must be configured as **openocd** and the ```device``` and ```configFiles``` according to your microcontroller and board. You can look for more information on the Cortex-Debug plugin's [wiki](https://github.com/Marus/cortex-debug/wiki){:target="_blank"}.
